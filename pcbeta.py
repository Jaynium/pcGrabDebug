            ...
            if 'i3-' in str(cpu_output) or 'i5-' in str(cpu_output) or 'i7-' in str(cpu_output) or 'i9-' in str(cpu_output):
                ...
                try:
                    ...
                    if genFind[0] == '1':
                        ...
                        gen = f'{genFind[0]}th'
            #if 'Celeron(R)' in str(cpu_output):
                #type = 'Celeron'
            #if Pentium(R) in str(cpu_output):
                #type = 'Pentium'
...
# ramPrompt = input('Did you need to insert Ram into device?: ')
# if ramPrompt == 'y':
#     ram_actual = '0 GB'
#     print(ram_actual)
...
# else:
#     caddyActual = "0"
# crackedScreenPrompt = str(input('Is the screen cracked? (y/n)'))
# if crackedScreenPrompt == 'y':
#     crackedScreenPrompt == "1"
# else:
#     crackedScreenPrompt == "0"
...
def sql_input():
    if isListable == "True":
        ...
        sqlCon = pymysql.connect(host="71.233.26.0", port=3306, user="Admin", password="Seekonk123",
                                 ...
                                 cursorclass=pymysql.cursors.DictCursor)
        cur = sqlCon.cursor()
        cur.execute(f"INSERT INTO `pc inventory` (`Brand`, `Model Pulled`, `Cpu Brand`, `Type Code`, `storage`, `Serial Number`, `ram`, `Caddy Present`, `Crack Grade`) VALUES ('{brandPc}', '{modelPc}','{brand}', '{type}-{code}', '{storageInfo}', '{serial}', '{ram_actual}', `{caddyActual}`, {crackedScreenPrompt})")
        sqlCon.commit()
        cur2 = sqlCon.cursor()
